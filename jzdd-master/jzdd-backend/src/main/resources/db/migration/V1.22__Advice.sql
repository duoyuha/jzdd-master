drop table if exists t_advice_order;

/*==============================================================*/
/* Table: S_CORP                                                */
/*==============================================================*/
create table t_advice_order
(
   ADVICE_ID              varchar(100) not null comment '本实体记录的唯一标识，产生规则为流水号',
   CORP_NO                varchar(32) comment '公司编号',
   ADVICE_NO             varchar(32) comment '订单编码',
   ...
   ADVICE_TYPE       varchar(8)  comment '报修类别 01咨询 02报修',
   ADVICE_TYPE_NAME       varchar(256)  comment '报修类别名称 01咨询 02报修',
   ADVICE_DESC       varchar(256)  comment '报修内容',
   ADVICE_REMARK       varchar(256)  comment '报修备注',
   ADVICE_TIME      datetime comment '咨询时间 yyyymmdd hhmmss',
  ...
  KEY `IDX12_CORP_NO` (`CORP_NO`) USING BTREE,
  KEY `IDX12_ORDER_NO` (`ORDER_NO`) USING BTREE,
  KEY `IDX12_SUPPLIER_NO` (`SUPPLIER_NO`) USING BTREE,
  KEY `IDX12_SUPPLIER_NAME` (`SUPPLIER_NAME`) USING BTREE,
  KEY `IDX12_ADVICE_TYPE` (`ADVICE_TYPE`) USING BTREE,
  KEY `IDX12_ADVICE_TIME` (`ADVICE_TIME`) USING BTREE,
  KEY `IDX12_SOLUTION_TIME` (`SOLUTION_TIME`) USING BTREE,
  KEY `IDX12_FINISH_FLG` (`FINISH_FLG`) USING BTREE,
  KEY `IDX12_IS_DEL` (`IS_DEL`) USING BTREE
)
ENGINE = InnoDB
